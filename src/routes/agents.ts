...
export function registerAgentRoutes(app: FastifyInstance, prisma: PrismaClient) {
  app.post<{ Body: RegisterBody }>(
    "/agents/register",
    // Unauthenticated and otherwise uncapped — each call creates a farm plus
    // its full tile grid, so it needs a much tighter limit than the global
    // default to prevent scripted registration from exhausting the DB.
    ...
    async (request, reply) => {
      ...
      // Farm+tile creation and agent creation commit together — if either
      // half fails, the other is rolled back instead of leaving an orphaned
      // farm.
