...
- Tools (see `src/mcp/tools.ts`): `inspect_farm`, `inspect_tile`, `inspect_inventory`, `till`, `plant`, `harvest`. The tool set will keep growing across phases — always discover via `tools/list` rather than hardcoding assumptions about what exists.
...
- **Inventory** (`src/game/inventory.ts`, `InventoryItem` model): per-farm counts keyed by item type — a `CropType` for seeds/produce, or a lowercased debris type (`weed`/`rock`) for what tilling turns up. `grantStartingInventory` runs inside `createFarmWithTiles` (so it's atomic with farm/tile creation) and seeds every new farm with `STARTING_SEED_COUNT` (5) of each crop plus `weed`/`rock` at 0, so the full item set is visible from farm creation. `plant` calls `consumeItem` first and fails the whole call (no tile mutation) if the agent is out of that crop's seeds — this is the "no magically planting whatever" gate. `till` and `harvest` call `addItem` after their tile mutation to deposit the cleared debris / harvested crop. No wallet/marketplace or capacity cap yet — inventory is unbounded ("infinite") and there's no way to spend/sell an item besides planting a seed.
